...
function GetFile {
    ...
    Param(
        ...
        [Parameter(HelpMessage='Optional extensions which you want to see.')]
        [ValidateSet("All Files (*.*) | *.*", "CSV | *.csv")]
...
function CopySPAccess {
    <#
        .Synopsis
        This function takes in a "source" and "target" users' email addresses and adds
    ...
    [CmdletBinding()]
    Param(
        [Parameter(HelpMessage='The source user''s O365 email')]
        [String]$SourceUser=(Read-Host -Prompt 'Please enter the user''s O365 email address'),
    ...
    # remove TargetUser user from all groups it is currently in
    Get-SPOUser -Site $SPSite -LoginName $TargetUser | select -ExpandProperty groups | % {Remove-SPOUser -Site $SPSite -LoginName $TargetUser -Group $_}

    # add test user to the groups of the chosen user
    Get-SPOUser -Site $SPSite -LoginName $SourceUser | select -ExpandProperty groups | % {Add-SPOUser -Site $SPSite -LoginName $TargetUser -Group $_}
}
